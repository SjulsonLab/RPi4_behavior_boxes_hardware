...
try:
    ...
    from display_mode_guard import (
        HeadlessDisplayModeError,
        require_headless_console_mode,
    )
...
def run_camera_setup_preview_hdmi_a1_smoke(
    *,
    ...
    try:
        ...
        elapsed_s = max(
            0.0,
            monotonic_fn() - (started_s if started_s is not None else monotonic_fn()),
        )
        ...
        summary.update(
            {
                ...
                "frame_prepare_time_avg_s": _average(
                    frame_prepare_time_total_s, frame_count
                ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--preview-source-mode", choices=("rgb_main", "yuv_lores"), default="rgb_main"
    )
