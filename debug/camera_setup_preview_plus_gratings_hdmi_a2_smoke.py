"""Dual-output setup preview smoke with camera on HDMI-A-1 and gratings on HDMI-A-2."""
...
class CameraSetupPreviewPlusGratingsSmokeFailure(RuntimeError):
    """Raised when the dual-output preview-plus-gratings smoke fails.
...
def compile_setup_grating_stimuli(
    *,
    repo_root: Path,
    ...
    """Load and compile default grating stimuli for the setup smoke.
    ...
    Returns:
        dict[str, Any]: Mapping from canonical stimulus names to compiled gratings.
    """
...
def _select_stimulus_frame(
    *,
    elapsed_s: float,
    compiled_go: Any,
    compiled_nogo: Any,
) -> tuple[str, np.ndarray]:
    """Select one grayscale stimulus frame for the current elapsed time.

    Args:
        elapsed_s: Elapsed smoke-loop time in seconds.
        compiled_go: Compiled go grating with ``frames`` and ``frame_interval_s``.
        compiled_nogo: Compiled nogo grating with ``frames`` and ``frame_interval_s``.

    Returns:
        tuple[str, np.ndarray]: Current stimulus label and grayscale frame with
        shape ``(height_px, width_px)``.
    """

    go_frame_count = int(np.asarray(compiled_go.frames).shape[0])
    nogo_frame_count = int(np.asarray(compiled_nogo.frames).shape[0])
    go_cycle_s = float(go_frame_count) * float(compiled_go.frame_interval_s)
    nogo_cycle_s = float(nogo_frame_count) * float(compiled_nogo.frame_interval_s)
    total_cycle_s = go_cycle_s + nogo_cycle_s
    phase_s = float(elapsed_s) % total_cycle_s if total_cycle_s > 0.0 else 0.0

    if phase_s < go_cycle_s:
        frame_index = min(
            int(phase_s / max(float(compiled_go.frame_interval_s), 1e-9)),
            go_frame_count - 1,
        )
        return "go_grating", np.asarray(compiled_go.frames[frame_index], dtype=np.uint8)

    phase_s -= go_cycle_s
    frame_index = min(
        int(phase_s / max(float(compiled_nogo.frame_interval_s), 1e-9)),
        nogo_frame_count - 1,
    )
    return "nogo_grating", np.asarray(compiled_nogo.frames[frame_index], dtype=np.uint8)


def run_camera_setup_preview_plus_gratings_hdmi_a2_smoke(
    *,
    output_root: Path,
    ...
    """Run a dual-output setup preview smoke with drifting gratings.
    ...
    if str(preview_source_mode) != "dmabuf_main":
        raise ValueError(
            "camera setup preview plus gratings dmabuf smoke supports only preview_source_mode='dmabuf_main'"
        )
    ...
    try:
        ...
        raise CameraSetupPreviewPlusGratingsSmokeFailure(
...
def main(argv: list[str] | None = None) -> int:
    """Run the dual-output preview-plus-gratings smoke as a CLI command.
    ...
    parser = argparse.ArgumentParser(
        description="Run a dual-output setup preview smoke with drifting gratings on HDMI-A-2."
    )
    ...
    try:
        ...
        return 2
    except CameraSetupPreviewPlusGratingsSmokeFailure as exc:
        print(f"Camera setup preview plus gratings smoke failed: {exc}", file=sys.stderr)
    ...
    print("Camera setup preview plus gratings smoke passed.")
