"""Single-output smoke for camera0 preview+recording with camera1 recording-only."""
...
try:
    from debug.camera_preview_recording_hdmi_a1_smoke import _average
    ...
    from camera_preview_recording_hdmi_a1_smoke import _average
...
class DualCameraPreviewRecordingSmokeFailure(RuntimeError):
    """Raised when the dual-camera preview+recording smoke fails.

    Attributes:
        summary: JSON-serializable failure summary containing the latest timing
            metrics and per-camera recording diagnostics gathered before the
            failure occurred.
    """
...
def run_dual_camera_preview_recording_hdmi_a1_smoke(
    *,
    output_root: Path,
    ...
    """Run a one-screen dual-camera smoke with preview on camera0 only.
    ...
    Returns:
        ...
        both cameras' recording artifact paths and timestamp counts.
    """
    ...
    try:
        ...
        preview_backend = preview_backend_factory(
            ...
            frame_rate_hz=float(frame_rate_hz),
            frame_release_fn=dual_source.release_frame,
        )
...
def main(argv: list[str] | None = None) -> int:
    """Run the dual-camera preview+recording smoke as a CLI command.
    ...
    parser = argparse.ArgumentParser(
        description="Run a dual-camera smoke with camera0 preview+recording and camera1 recording-only."
    )
    ...
    try:
        ...
        print(f"Dual-camera preview recording smoke failed: {exc}", file=sys.stderr)
    ...
    print("Dual-camera preview recording smoke passed.")
