"""Single-output dmabuf camera setup preview smoke for HDMI-A-1."""
...
class CameraSetupPreviewDmabufSmokeFailure(RuntimeError):
    """Raised when the dmabuf preview smoke fails after partial startup.

    Attributes:
        summary: JSON-serializable failure summary containing the latest timing
            metrics and DRM diagnostics gathered before the failure.
    """
...
def run_camera_setup_preview_hdmi_a1_dmabuf_smoke(
    *,
    output_root: Path,
    ...
    """Run a single-output HDMI-A-1 dmabuf preview smoke.

    Data contracts:

    - ``resolution_px`` is ``(width_px, height_px)`` in display pixels.
    - ``frame_source_factory`` must return an object exposing:
      - ``capture_frame_for_preview() -> frame`` where ``frame`` carries dmabuf metadata
    ...
    Args:
        ...
        request_mode: Preview request-selection policy. ``"next"`` consumes
            completed frames in order and is the default because it behaves
            best in the shared-DRM dmabuf preview path. ``"latest"`` uses
            ``flush=True`` to prefer fresher frames.
        ...
        frame_source_factory: Factory returning a dmabuf-capable camera source.
        preview_backend_factory: Factory returning a dmabuf-capable preview backend.
            The backend may accept ``frame_release_fn`` and become responsible
            for releasing previously displayed frames.
        monotonic_fn: Monotonic clock callable returning seconds.
        sleep_fn: Optional sleep callable retained for compatibility with the
            older loop signature. The latest-frame preview path does not rely on
            fixed loop sleeps for pacing.

    Returns:
        dict[str, object]: JSON-serializable summary with preview timing and
        DRM diagnostics.
    """
    ...
    try:
        ...
        if preview_backend is not None and hasattr(preview_backend, "diagnostics"):
            summary["preview_drm_diagnostics"] = preview_backend.diagnostics()
        raise CameraSetupPreviewDmabufSmokeFailure(
...
def main(argv: list[str] | None = None) -> int:
    """Run the HDMI-A-1 dmabuf preview smoke as a CLI command.
    ...
    parser = argparse.ArgumentParser(
        description="Run a single-output dmabuf camera setup preview smoke on HDMI-A-1."
    )
    ...
    try:
        ...
        return 2
    except CameraSetupPreviewDmabufSmokeFailure as exc:
        print(f"Camera setup preview dmabuf smoke failed: {exc}", file=sys.stderr)
    ...
    print("Camera setup preview dmabuf smoke passed.")
